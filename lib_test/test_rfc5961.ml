...
let reply_id_from ~src ~dst data =
  ...
  WIRE.{remote_port = sport; remote_address = src ; local_port = dport; local_address = dst }
...
let sut_connects_and_remains_connected stack fail_callback =
  let conn = VNETIF_STACK.Stackv4.TCP.create_connection (VNETIF_STACK.Stackv4.tcp stack) in
  ...
  Lwt.pick [
    (VNETIF_STACK.Stackv4.TCP.read flow >>= fail_result_not_expected fail_callback);
  ...
  let fsm ip state ~src ~dst data =
    ...
    | `WAIT_FOR_SYN ->
      ...
      if syn then (
        let id = reply_id_from ~src ~dst data in
        (* refused *)
        WIRE.xmit ~ip ~id ~rst:true ~rx_ack:(ack data) ~seq:(Sequence.of_int32
                                                               0l) ~window
          ~options (Cstruct.create 0) >>= fun () ->
        Lwt.return Fsm_done
      ) else
    ...
    | `WAIT_FOR_ACK ->
      if Tcp_wire.get_ack data then (
        ...
        WIRE.xmit ~ip ~id ~rst:true ~rx_ack:None ~seq:(Sequence.of_int32 10l)
          ~window ~options (Cstruct.create 0) >>= fun () ->
...
let rst_on_established_scenario =
  ...
  let sut stack fail_callback =
    let conn = VNETIF_STACK.Stackv4.TCP.create_connection (VNETIF_STACK.Stackv4.tcp stack) in
    or_error "connect" conn (server_ip, 80) >>= fun flow ->
    VNETIF_STACK.Stackv4.TCP.read flow >>= function
    | `Eof ->
